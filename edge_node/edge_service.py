...
class EdgeService:
    ...
    def init_rabbitmq() -> None:
        ...
        # --- Вспомогательная функция: подключение с повторными попытками ---
        def _connect_with_retry(host: str, max_attempts: int = 30, delay: int = 2):
            """
            Tries to connect to RabbitMQ with retries.
            Raises the exception after all attempts fail.
            """
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        logger.warning(
                            f"RabbitMQ connection attempt {attempt + 1}/{max_attempts} failed: {e}. Retrying in {delay}s..."
                        )
                        time.sleep(delay)
                    else:
                        logger.error(f"Failed to connect to RabbitMQ at {host} after {max_attempts} attempts: {e}")
                        raise  # Re-raise after all attempts exhausted

        # === Подключение к RabbitMQ для общения с Fog ===
        connection = _connect_with_retry(EdgeService.FOG_RABBITMQ_HOST)
    ...
    def _process_message(message: dict) -> dict:
        logger.info(
            f"[EDGE DEBUG] _process_message called: scope={message.get('scope')}, start_date={message.get('start_date')}, current_date={message.get('current_date')}")
        logger.info(
    ...
    def set_completed_previous_round_flag(flag: bool):
        ...
        current_node = NodeState.get_current_node()
        if not current_node:
            logger.error("Cannot set flag: current_node is None")
            return
        ...
        data = {
            ...
            # === ИСПРАВЛЕНИЕ: конвертировать Enum в значение ===
            'fed_node_type': current_node.fed_node_type.value if hasattr(current_node.fed_node_type, 'value') else str(
                current_node.fed_node_type),
            'ip_address': current_node.ip_address,
            'port': current_node.port,
            'parent_node_id': current_node.parent_node.id if current_node.parent_node else None,
            'parent_node_name': current_node.parent_node.name if current_node.parent_node else None,
            # === ИСПРАВЛЕНИЕ: конвертировать Enum в значение ===
            'parent_node_fed_node_type': current_node.parent_node.fed_node_type.value if current_node.parent_node and hasattr(
                current_node.parent_node.fed_node_type, 'value') else str(
                current_node.parent_node.fed_node_type) if current_node.parent_node else None,
            'parent_node_ip_address': current_node.parent_node.ip_address if current_node.parent_node else None,
            'parent_node_port': current_node.parent_node.port if current_node.parent_node else None
        }
