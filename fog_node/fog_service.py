...
class FogService:
    ...
    def init_rabbitmq() -> None:
        ...
        # === Первое подключение: Cloud-Fog communication ===
        connection = _connect_with_retry(FogService.CLOUD_RABBITMQ_HOST)
        ...
        connection.close()
        logger.info(f"Successfully connected to RabbitMQ at {FogService.CLOUD_RABBITMQ_HOST}")

        # === Второе подключение: Fog-Edge communication ===
        connection = _connect_with_retry(FogService.FOG_RABBITMQ_HOST)
        channel = connection.channel()
        ...
        connection.close()
        logger.info(f"Successfully connected to RabbitMQ for Edge communication at {FogService.FOG_RABBITMQ_HOST}")
