...
class CloudService:
    ...
    def execute_training_process(start_date: str | None, end_date: str, is_cache_active: bool,
        ...
        if NodeState.get_current_node() is None:
        ...
        while True:
            ...
            try:
                ...
                def callback(ch, _method, _properties, body):
                    try:  # ← ДОБАВИТЬ ОТКРЫТИЕ try ЗДЕСЬ
                        ...
                        if int(message.get("scope")) == MessageScope.TRAINING.value:
                            ...
                            logger.warning(
                                f"Received message from fog with scope {message.get('scope')} being not recognized.")

                    except AttributeError as e:  # ← ДОБАВИТЬ except ЗДЕСЬ
                        if "'NoneType' object has no attribute 'save'" in str(e):
                            logger.warning(f"Model not initialized yet, skipping save: {e}")
                        else:
                            logger.error(f"AttributeError in callback: {e}")
                    except Exception as e:  # ← И общий except
    ...
    def get_fog_test_result(message):
        ...
        logger.info(f"[DEBUG] get_fog_test_result called. Buffer: {CloudService.buffer}")
        logger.info(f"[DEBUG] evaluation_received_results_counter: {CloudService.evaluation_received_results_counter}")
        logger.info(
            f"[DEBUG] child_nodes count: {len(NodeState.get_current_node().child_nodes) if NodeState.get_current_node() else 0}")
        # =============================
